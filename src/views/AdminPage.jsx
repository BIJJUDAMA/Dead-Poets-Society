...
import { FileText, Inbox, Users, Trash, Shield, ShieldOff, Edit, X, Eye, Check, Loader2, SlidersHorizontal, Search } from 'lucide-react';
...
const EditPoemForm = ({ note, onSave, onCancel, isSaving }) => {
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden pt-2">
            ...
            <div className="flex-1 overflow-y-auto px-1 pr-2 py-1 space-y-4">
                {/* 2-column grid on desktop for Title & Poet Name */}
            ...
            <div className="pt-3 pb-1 border-t border-stone-900 mt-2 flex items-center justify-end gap-2.5 flex-shrink-0">
                <Button variant="ghost" onClick={onCancel} disabled={isSaving} className="text-stone-400 hover:text-white rounded-xl h-10 px-4">
                    Cancel
                </Button>
                <Button 
                    onClick={() => onSave(formData)} 
                    disabled={isSaving}
                    className="bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 text-stone-950 font-bold rounded-xl px-6 h-10 shadow-md active:scale-95"
                >
                    {isSaving ? 'Saving...' : 'Save Changes'}
                </Button>
            </div>

            {/* Theme Selector Modal inside Edit Poem */}
            {isThemePickerOpen && (
